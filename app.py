...
# ページの設定（タイトルやレイアウト）
st.set_page_config(
    page_title="徳島国際工科大学（TITC）", layout="wide"
)
...
if page == "トップページ":
    ...
    with col1:
        st.write("### 大学からのお知らせ")
        st.write("・2026/07/15: 常三島ベイキャンパス 最先端AIラボの竣工式について")
        st.write("・2026/07/01: バイオ・スマート農工学部 LEDアグリテック技術プレスリリース")
        st.write("・2026/06/20: 2027年度 総合型選抜・学校推薦型選抜の募集要項公開")

    with col2:
        st.write("### 重要なお知らせ（学内者向け）")
        with st.expander("【蔵本】キャンパス間シャトルバスのダイヤ改正について"):
            st.write("蔵本〜常三島ベイキャンパス間の平日運行便数が変更となります。詳細は学内掲示板を確認してください。")
...
# --- ページ2：大学概要・沿革 ---
    ...
    st.write("### 沿革")
    history_data = {
        "年号": ["明治8年 (1875)", "明治34年 (1901)", "昭和24年 (1949)", "平成20年 (2008)"],
        "事項": [
...
# --- ページ3：学部・学科紹介 ---
    ...
    with tab3:
        ...
        st.write("・阿波イノベーション農工学科（LEDアグリテック・発酵工学・植物成分分析）")
...
# --- ページ4：キャンパスライフ・学食 ---
    ...
    st.write("### 学生食堂『阿波らんち』")
    st.info("食堂人気メニュー：徳島ラーメンセット（藍ドレッシングサラダ付） / 450円")

    st.write("### 在学生メッセージ")
    st.text("「3年次のキャンパス移動に際し、蔵本から常三島への転居か吉野川沿いの通学かを検討することになります」（工学部 3年）")
    st.text("「キャンパス内には海外からの留学生も多く、日常的に異文化交流が行われています」（国際フロンティア工学部 2年）")
    st.text("「農工学科の研究用LED植物工場で収穫された野菜が学食で提供されることがあります」（バイオ・スマート農工学部 4年）")
